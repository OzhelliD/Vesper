...
def tts_generate():
    """Genera audio MP3 con gTTS (Google Translate Text-to-Speech)
    ...
    try:
        ...
        # Generar audio con gTTS - voz natural en español de México
        ...
        return Response(mp3.getvalue(), mimetype="audio/mpeg", 
...
def get_alarms():
    """Lista alarmas del usuario actual"""
    try:
        user_id = request.headers.get("X-User-ID", "unknown")
        if not alarm_manager:
            return jsonify({"error": "Alarm manager no iniciado"}), 503
        
        result = alarm_manager.list_alarms(user_id)
...
def create_alarm():
    ...
    try:
        ...
        result = alarm_manager.create_alarm(user_id, alarm_time, is_recurring)
        return jsonify(result), (200 if result.get("success") else 400)
    except Exception as e:
        return jsonify({"error": str(e)}), 500

@app.route("/api/alarms/delete/<int:alarm_id>", methods=["DELETE"])
def delete_alarm(alarm_id):
    ...
    try:
        ...
        result = alarm_manager.delete_alarm(alarm_id, user_id)
        return jsonify(result)
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def reminder_scheduler():
    ...
    while True:
        try:
            ...
            # Alarmas — usar nuevo AlarmManager
            if alarm_manager:
                try:
                    alarms_to_fire = alarm_manager.check_alarms()
                    for alarm in alarms_to_fire:
                        ...
                        label = f"Alarma {alarm_time}: {song}"
                        alarm_manager.mark_triggered(aid)
